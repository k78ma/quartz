...
- An [[Activation Function|activation function]] $f: \mathbb{R} \to \mathbb{R}$, which gives us non-linearity.
...
## Training
How do we train a single unit? Given a loss function $L(guess, actual)$, and a dataset $\{ (x^{(1)}, y^{(1)}), \dots, (x^{n}, y^{(n)}) \}$, we can do [[Gradient Descent|gradient descent]], adjusting the weights $w, w_{0}$ to minimize:
$$
